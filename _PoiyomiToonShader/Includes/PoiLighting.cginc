    ...
    void calculateBasePassLighting(float3 normal, float2 uv)
    {
        ...
        float bw_directLighting = (((dot(poiLight.direction, normal) * 0.5 + 0.5) * bw_lightColor * lerp(1, poiLight.attenuation, _AttenuationMultiplier)) + dot(ShadeSH9Normal(normal), grayscale_vector));
    ...
    void calculateLighting(v2f i)
    {
        ...
        poiLight.color = _LightColor0.rgb;
            ...
            calculateBasePassLighting(i.normal, i.uv);
                ...
                float nDotL = dot(i.normal, poiLight.direction);
                poiLight.finalLighting = poiLight.color * poiLight.attenuation * smoothstep(.5 -_AdditiveSoftness + _AdditiveOffset, .5 + _AdditiveSoftness + _AdditiveOffset, .5 * nDotL + .5);
            #endif
        #endif
    }
