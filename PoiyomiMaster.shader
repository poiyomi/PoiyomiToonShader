...
Shader ".poiyomi/Master/Opaque"
{
    Properties
    {
        ...
        [Header(RGB Texture Mask)]
        [Toggle(_RGB_MASK)]_RGB_MASK ("Enable RGB Mask", Float) = 0
        [Toggle(_USE_UV2)]_USE_UV2 ("Use UV2", Float) = 0
        ...
        [Header(Blinking Emission(Requires Emission))]
        ...
        [Header(Scrolling Emission(Requires Emission))]
        ...
        _SpecularStrength ("Specular Strength", Range(0, 5)) = .25
        _SpecularSize ("Specular Size", Range(0, 1)) = .025
        _SpecularNormalMultiplier ("Normal Multiplier", Float) = 1
        
        [Space][Header(Rim Lighting)]
        ...
        [Space]
        _RimGlowNormalMultiplier ("Normal Multiplier", Float) = 1
        
        [Space][Header(Texture Blending)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _SpecularNormalMultiplier;
            ...
            float _LightingNormalMultiplier;
            ...
            float _RimGlowNormalMultiplier;
            float _RimTexTile;
            float4 _RimColor;
            ...
            fixed4 frag(v2f i, float facing: VFACE): SV_Target
            {
                ...
                col.rgb *= _Color.rgb;
                    ...
                    fixed4 rgbMask_var = tex2D(_RGBMask, TRANSFORM_TEX(i.uv2, _RGBMask));
                    fixed4 redTexture_var = tex2D(_RedTexture, TRANSFORM_TEX(i.uv2, _RedTexture));
                    fixed4 greenTexture_var = tex2D(_GreenTexture, TRANSFORM_TEX(i.uv2, _GreenTexture));
                    fixed4 blueTexture_var = tex2D(_BlueTexture, TRANSFORM_TEX(i.uv2, _BlueTexture));
                ...
                clip(col.a - _Clip);
                    ...
                    float3 cameraVertDot = abs(dot(i.cameraToVert, normalDirection * _RimGlowNormalMultiplier));
                    #ifdef _RIMGLOW_SOFT
                    ...
                    float FakeLight = (dot(normalDirection * _LightingNormalMultiplier, normalize(_LightDirection)) + 1) / 2;
                    float4 LightColor = tex2D(_LightingGradient, float2(FakeLight, 0));
                    
                    float4 specular = 0;
                    #ifdef _SPECULAR_HIGHLIGHTS
                        float specularStrength = tex2D(_SpecularMap, TRANSFORM_TEX(i.uv, _SpecularMap));
                        float3 reflectionVector = reflect(i.cameraToVert, normalDirection * _SpecularNormalMultiplier);
                        float specularDot = 1 - ((dot(reflectionVector, normalize(_LightDirection)) + 1) / 2);
                        float4 specularColor = lerp(col, _SpecularColor, _SpecularBias);
                        if (specularDot > 1 - _SpecularSize)
                        {
                            float specularRamp = tex2D(_SpecularRamp, float2(FakeLight * _SpecularSize, .5));
                            specular = specularColor * _SpecularStrength * specularRamp * specularStrength;
                        }
                    #endif
