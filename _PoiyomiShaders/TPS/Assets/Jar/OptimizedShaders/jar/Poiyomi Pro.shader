Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/539cf1b63d2e8ee4da5bcb670cbed48b"
{
	Properties
	{
...
#define PROP_INVERSE_CLIPPING 1
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(3) == 1)
				{
					...
					if (float(0) && float(0))
					{
						...
						poiFragData.alpha = (poiFragData.alpha - float(0)) / max(fwidth(poiFragData.alpha), 0.0001) + float(0);
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1) > 0)
				{
					...
					specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * poiLight.occlusion * attenuation * float(1);
					#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						fresnelTerm = 1;
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * poiLight.occlusion * poiLight.vAttenuation[index] * float(1);
					}
					#endif
				}
				if (float(0) == 1 && float(1) > 0)
				...
				if (float(1) > 0)
				{
					...
					float reflStr = reflectionMask * float(1);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.7615293,0.7615293,0.7615293,0.2156863).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.7615293,0.7615293,0.7615293,0.2156863).a;
