    ...
    */
    ...
    float3 getClearcoatIndirectSpecular(float metallic, float roughness, float3 reflDir, float3 worldPos, float3 lightmap, float3 normal)
    {
        float3 spec = float3(0, 0, 0);
            ...
            {
                indirectSpecular = texCUBElod(_ClearcoatFallback, float4(envData.reflUVW, roughness * UNITY_SPECCUBE_LOD_STEPS)) * poiLight.finalLighting;
            }
    ...
    void calculateAndApplyClearCoat(inout float4 finalColor)
    {
        ...
        float3 creflViewDir = getClearcoatAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent, poiMesh.normals[0], clearcoatRoughness, _ClearcoatAnisotropy);
        ...
        float3 vDirectSpecular = 0;
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                float3 v0directSpecular = getClearcoatDirectSpecular(clearcoatRoughness, vcndh, cvdn, vcndl, saturate(poiLight.vDotLH[index]), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, _ClearcoatAnisotropy) * poiLight.vAttenuation * vcndl * poiLight.vColor[index];
                vDirectSpecular += v0directSpecular;
            }
        #endif
        
        float3 clearcoatDirectSpecular = getClearcoatDirectSpecular(clearcoatRoughness, cndh, cvdn, attenuation, saturate(poiLight.lDotH), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, _ClearcoatAnisotropy) * poiLight.attenuation * attenuation * poiLight.color;
