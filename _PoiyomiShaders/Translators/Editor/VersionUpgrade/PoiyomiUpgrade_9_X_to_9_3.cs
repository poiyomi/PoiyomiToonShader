...
namespace Poi.Tools.ShaderTranslator.VersionUpgrade
{
	public static class PoiyomiUpgrade_9_X_to_9_3
	{
		...
		{
			// Before unlocking: snapshot the (non-stripped) serialized values, then retarget any stripped-texture
    		// tag (e.g. _ClippingMask, iridescence ramp) so it restores onto its 9.3 name during unlock.
			var snapshot = LegacyMaterialDetector.MaterialSerializedReader.Read(material);
			PoiyomiLegacyRemaps.RenameStrippedTextureTags(material, snapshot);
			...
			// Now on 9.3: apply the 8.x property renames, fix keywords (an 8.x->matcap migration enables a matcap
    		// feature that needs its keyword), then drop the removed DPS/aniso/8.x orphans (VRAM).
		...
		public static void UpgradeMaterials(IEnumerable<Material> materials)
		{
			...
			try
			{
				foreach (Material material in materials)
				{
					...
					if (info.IsLocked)
					{
						// Snapshot values, then retarget stripped-texture tags, BEFORE the grouped unlock.
						var snap = LegacyMaterialDetector.MaterialSerializedReader.Read(material);
						snapshots[material] = snap;
						PoiyomiLegacyRemaps.RenameStrippedTextureTags(material, snap);
						SpoofRecoveryTags(material, target);
