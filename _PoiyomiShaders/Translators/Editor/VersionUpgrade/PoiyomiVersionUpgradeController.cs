...
namespace Poi.Tools.ShaderTranslator.VersionUpgrade
{
	...
	public static class PoiyomiVersionUpgradeController
	{
		...
		public static bool UpgradeToLatest(Material material, bool deferFinalShaderSwap, out Shader finalShader)
		{
			...
			// Pre-9.3 materials (8.x, or removed 9.0-9.2) can't be fed to the ScriptedShaderTranslator chain directly -
			// their source shader is gone or too old. Land them on a present 9.3 shader first via the dedicated legacy
			// pipeline (handles unlock, 8.x property remaps, iridescence->matcap, and error-shader recovery), then fall
			// through to the normal 9.3 -> 10.0 chain below. Detected even when the current shader is missing/mis-resolving.
			if (LegacyMaterialDetector.TryDetectLegacyNine(material, out LegacyMaterialInfo legacyInfo))
			{
				if (!PoiyomiUpgrade_9_X_to_9_3.UpgradeToNine3(material, legacyInfo))
				return false;
				// Material is now on a present 9.3 shader (unlocked, remaps applied) - continue to chain it to 10.0.
			}
		...
		public static bool UpgradeAcrossShaderSwap(Material material, Shader oldShader, Shader newShader)
		{
			...
			// Pre-9.3 source (8.x, or removed 9.0-9.2): there's no direct ScriptedShaderTranslator. Land the material on
			// 9.3 first via the legacy pipeline (the 8.x values are still serialized), then continue the normal chain up
			// to the *picked* version - so swapping to a 9.3 shader stops at 9.3, and swapping to 10.0 chains all the way.
			if (oldVersion < new Version(9, 3))
			{
				if (!LegacyMaterialDetector.TryDetectLegacyNine(material, out LegacyMaterialInfo legacyInfo))
          			return false;
				
				material.shader = oldShader; // restore so the 8.x layout is live for the remaps
				if (!PoiyomiUpgrade_9_X_to_9_3.UpgradeToNine3(material, legacyInfo))
					return false;

				// The user picked exactly 9.3 - honor their exact shader choice and stop (same-version swap, values carry).
				if (newVersion <= new Version(9, 3))
				{
					ScriptedShaderTranslator.ApplyDeferredShaderSwap(material, newShader);
					return true;
				}

				// Otherwise fall through to chain 9.3 -> newVersion onto newShader, re-pointing the source at 9.3.
				oldShader = material.shader;
				oldVersion = new Version(9, 3);
			}
		...
		public static void UpgradeMaterials(IEnumerable<Material> materials)
		{
			...
			try
			{
				// Legacy (pre-9.3) materials have to go through the legacy pipeline BEFORE anything unlocks them: it
				// repoints each locked material's OriginalShader recovery tags at its 9.3 target first, so the unlock
				// restores onto 9.3. Unlocking them here instead handed them to ShaderOptimizer's generic recovery,
				// which resolves a stale ".poiyomi/Poiyomi Toon" tag straight onto the 10.0 shader or, when that tag
				// no longer resolves, Levenshtein-guesses across every shader in the project - so having Poiyomi Pro
				// installed pulled Toon materials onto Pro variants. Either way the material landed on a present
				// shader with none of its values translated, and the loop below then skipped it as "already latest".
				// This is why the per-material button worked while the bulk utility did not: UpgradeToLatest unlocks
				// only AFTER its legacy step, and PoiyomiUpgrade_9_X_to_9_3 batches its own grouped unlock correctly.
				var legacyMaterials = materialList.Where(m => m != null && LegacyMaterialDetector.NeedsLegacyUpgrade(m)).ToList();
				if (legacyMaterials.Count > 0) PoiyomiUpgrade_9_X_to_9_3.UpgradeMaterials(legacyMaterials);

				// Whatever is still locked is 9.3+, so it can take the batched unlock (one compile pass for all of
				// them) instead of being unlocked one at a time inside the loop.
